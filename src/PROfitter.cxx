...
float PROfitter::FitScan(PROmetric &metric,
                          const std::vector<Eigen::VectorXf> &seeds,
                          float dedup_eps_phys)
{
    // Scan-mode timing instrumentation: same accumulator as Fit() so the
    // --profile-timing summary reports a unified picture across both paths.
    // Latin/PSO fractions naturally drop to ~0 when FitScan dominates a run.
    const bool tim_on = PROfit::GetScanTimingEnabled();
    const auto fit_t0 = tim_on ? std::chrono::steady_clock::now()
                               : std::chrono::steady_clock::time_point{};

    if (seeds.empty()) {
        log<LOG_ERROR>(L"%1% || FitScan called with no seeds; returning sentinel chi².") % __func__;
        return 1e10f;
    }

    const long nphys = (long)metric.GetModel().nparams;

    // Build the deduped seed list. Convention (set by PRObe::warm_seeds_for):
    //   seeds[0]    = global BF (always present).
    //   seeds.back() = warm-start pool entry from PRObe (present once pool is populated).
    //   seeds[1..N-2] = harmonic freq-seed points (may share physics with the BF).
    //
    // We *always* keep seeds.front() and seeds.back() regardless of physics
    // distance. Rationale: in nuisance scans the pool entry's physics coincides
    // with the BF (same physics minimum), but its *nuisance* values are tuned
    // for the local θ slice and are a far better starting point than the BF's
    // global-optimum nuisances. A physics-only dedup was dropping it, forcing
    // LBFGS to walk all nuisances every fit — produced jagged, unconverged
    // chi² values across nuisance scans. Force-keeping the warm-start fixes it.
    //
    // Middle seeds (freq-seed points) are still deduped against the kept set
    // on physics-param L2 distance, since two harmonic seeds in the same
    // physics basin really would yield identical LBFGS results.
    std::vector<Eigen::VectorXf> deduped;
    deduped.reserve(seeds.size());
    deduped.push_back(seeds.front());                          // BF
    for (size_t i = 1; i + 1 < seeds.size(); ++i) {            // middle = freq seeds
        const auto &s = seeds[i];
        if (s.size() < nphys) continue;
        bool is_dup = false;
        for (const auto &d : deduped) {
            const float dist = (s.head(nphys) - d.head(nphys)).norm();
            if (dist < dedup_eps_phys) { is_dup = true; break; }
        }
        if (!is_dup) deduped.push_back(s);
    }
    if (seeds.size() >= 2) {
        deduped.push_back(seeds.back());                       // warm-start pool entry
    }

    log<LOG_INFO>(L"%1% || FitScan: seeds_in=%2%, kept=%3% (front/BF + back/pool always kept; middle deduped, eps=%4%).")
        % __func__ % seeds.size() % deduped.size() % dedup_eps_phys;
    if (seeds.size() >= 2) {
        // Per-seed diagnostic: physics-prefix L2 of each kept seed relative to BF.
        // Helps confirm visually that BF and the pool entry are both present and
        // the freq-seeds occupy distinct basins.
        for (size_t k = 0; k < deduped.size(); ++k) {
            const float d_to_bf = (k == 0) ? 0.0f
                : (deduped[k].head(nphys) - deduped.front().head(nphys)).norm();
            log<LOG_DEBUG>(L"%1% || FitScan seed %2%: phys-distance to BF = %3%")
                % __func__ % k % d_to_bf;
        }
    }

    const auto lbfgs_t0 = tim_on ? std::chrono::steady_clock::now()
                                 : std::chrono::steady_clock::time_point{};
    ...
    for (const auto &seed : deduped) {
        x = seed;
        // Clamp into bounds. Fixed-coordinate parameters (lb==ub) get pinned
        // to the fixed value so LBFGS doesn't waste an iteration on the boundary.
        for (long i = 0; i < x.size(); ++i) {
            if (std::abs(ub(i) - lb(i)) < 1e-10f) {
                x(i) = lb(i);
            } else {
                if (!std::isinf(lb(i)) && x(i) < lb(i)) x(i) = lb(i);
                if (!std::isinf(ub(i)) && x(i) > ub(i)) x(i) = ub(i);
            }
        }
        float fx = 1e10f;
        try {
            const int niter = solver.minimize(metric, x, fx, lb, ub);
            (void)niter;
            if (fx < chimin) {
                chimin = fx;
                best_fit = x;
            ...
            exception_string_map[msg]++;
            log<LOG_WARNING>(L"%1% || FitScan: LBFGS minimize failed: %2%") % __func__ % msg.c_str();
        }
    }
    ...
    if (!any_success) {
        log<LOG_ERROR>(L"%1% || FitScan: all %2% LBFGS calls failed; returning sentinel chi².")
            % __func__ % deduped.size();
        return 1e10f;
    }
