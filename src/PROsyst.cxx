...
namespace PROfit {
    ...
    PROsyst::PROsyst( const PROpeller &prop, const PROconfig &config, const std::vector<SystStruct>& systs, bool shapeonly, int other_index) : other_index(other_index) {
        ...
        if(config.m_use_mcstats){
            Eigen::MatrixXf fractional_mcstat_cov = other_index < 0 ? prop.mcStatErr.array().square().matrix().asDiagonal()
                : prop.otherMCStatErr[other_index].array().square().matrix().asDiagonal();
