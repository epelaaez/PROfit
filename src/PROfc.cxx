...
void fc_worker(fc_args args) {
    ...
    for(size_t u = 0; u < args.todo; ++u) {
        ...
        Eigen::VectorXf throwC = Eigen::VectorXf::Constant(args.config.m_num_variable_bins_total[args.config.i_prime]_collapsed, 0);
        for(size_t i = 0; i < args.systs.GetNSplines(); i++)
            throws(i+nphys) = d(rng);
        for(size_t i = 0; i < args.config.m_num_variable_bins_total[args.config.i_prime]_collapsed; i++)
            throwC(i) = d(rng);
