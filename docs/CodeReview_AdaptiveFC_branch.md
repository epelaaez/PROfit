...
| F2 | `enqueue_eval` dedups by linear scan of the whole work deque (O(queue²)); `enforce_balance_around` scans **all** cells per subdivision; the dense chi² reconstruction runs for every throw. | FIXED (first two) — pending-key `unordered_set` for O(1) double-queue checks; the 2:1 balance check locates the containing leaf via an anchor→cells hash map in O(levels) probes instead of an O(cells) scan. The dense reconstruction is deliberately kept: it is consumed by the always-on build-mesh diagnostics (`write_slice1_diagnostics`), not dead weight. |
| F3 | `generate_throws` retains every throw's full `AMRResult` (including `bestfit_map`, pure warm-start scratch) in RAM for the whole run — linear memory growth in `n_throws`. | FIXED — `bestfit_map` (one full parameter vector per evaluated point, by far the largest piece) is freed per throw; leaves/chi² are retained because the meta-mesh tally and diagnostics genuinely read them. |
| F4 | Brazil throws rebuild the CV spectrum + `DecomposeFractionalCovariance` (an SVD) **inside** the per-throw loop although they are throw-invariant (`generate_throws` hoists the same computation); IDW surfaces recomputed per CL page and again on ROOT save. | FIXED (hoist) — the CV spectrum and decomposition are computed once before the brazil loop and passed in. The per-page IDW recomputation is plotting-only cost and was left as-is (flagged under code quality with the duplicated IDW builders). |
| F5 | `run_one_pe` heap-allocates a fresh metric for every PE (thousands per cell); the wilks prepass demonstrates `thread_local` metric reuse. | DOC — assessed and deliberately not changed: each PE runs two full `PROfitter::Fit` calls (hundreds to thousands of metric evaluations each), so the one metric construction per PE is noise; reuse would also require a data-swap API on PROmetric since each PE carries new fake data. |
