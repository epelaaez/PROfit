...
| T3.34 | FC bank stores `chi2_syst/chi2_osc/dchi2` as `float`; `dchi2` is a difference of large near-equal values, so quantile comparisons ride on ~0.01–0.1 noise near critical values. | `inc/PROAdaptiveFC.h:239–241` | FIXED — doubles in `PEBankRecord` and the observed-dchi² path; bank artifact version bumped (old banks are rejected with a clear message). |
...
| H7 | Per-call allocation trims: PROchi wrote a dense N×N stat covariance member every call (used only in NaN diagnostics); PROCNP allocated a dense zero N×N every call for a diagonal; `Pull()` re-derived priors² each call; `maybe_convert_log` did a string-temporary + hash-map lookup + `pow` per call; `get_probs` allocated a fresh matrix per call. | FIXED — stat covariances kept as vectors (dense only for the NaN diagnostic), priors² precomputed, log-conversion by precomputed per-index flag, probability buffer reused. |
